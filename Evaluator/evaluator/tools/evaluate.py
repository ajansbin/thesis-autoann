from zod.eval.detection._nuscenes_eval.common.data_classes import EvalBoxes, EvalBox
...
import numpy as np
from evaluator.tools.evaluate_nuscenes_style import (
    evaluate_nuscenes_style,
    _nuscenes_evaluate,
    _filter_eval_boxes_on_ranges,
)
...
from zod.eval.detection._nuscenes_eval.detection.data_classes import DetectionMetricData
...
def evaluate(
    ...
    gtsEval, predEval = create_eval_boxes(tracking_results, result_type, conf)
    if eval_type == "nuscenes":
        evaluate_nuscenes_style(
            ...
            verify_coordinate_system=False,
        )
    elif eval_type == "giou":
...
def create_eval_boxes(tracking_results, result_type, conf, seqs=None, score_thresh=0.0):
    ...
    for seq_id, gt_frame in gt_frames.items():
        if seqs and seq_id not in seqs:
            continue
        ...
        for box in tracking_results.get_pred_boxes_from_frame(gt_frame):
            if conf["data"]["remove_bottom_center"]:
                print("bottom center true")
                gravity_center = box["translation"][-1] + box["size"][-1] / 2
                translation = (
                    [box["translation"][0]] + [box["translation"][1]] + [gravity_center]
                )
            else:
            ...
            name = (
                box["detection_name"]
                if result_type == "detection"
                else box["tracking_name"]
            )
            if name not in class_names:
                continue
            # must set negative size to 0
            box["size"] = np.absolute(box["size"])  # Why not max(0, box["size"])?
        ...
        for box in gts[seq_id]:
            ...
            box3d = Box3D(
                box["translation"], box["size"], box["rotation"], Lidar.VELODYNE
            )
            box3d.convert_to(EGO, seq.calibration)
    ...
    print(f"Ignored {count} tracks since score < 0.5")
...
def calculate_giou3d(gt_box: EvalBox, pred_box: EvalBox):
    gt_array = list(gt_box.translation) + list(gt_box.size) + list(gt_box.rotation)
    pred_array = (
        list(pred_box.translation) + list(pred_box.size) + list(pred_box.rotation)
    )
    giou = giou3d(gt_array, pred_array)
    return giou
...
def giou3d(gt, pred):
    ...
    box = Box3D(center, size, rotation, Lidar)

    gt_box = Box3D(gt_center, gt_size, gt_rotation, Lidar)
...
def get_nuscenes_eval_metrics(
    gt_boxes: EvalBoxes, det_boxes: EvalBoxes, class_ranges: list = [250]
) -> Dict[str, float]:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--result-path",
        type=str,
        default="/staging/agp/masterthesis/2023autoann/storage/detection/CenterPoint/predictions/cp-zod-mini-results-val/pts_bbox/results_zod.json",
    ...
    parser.add_argument(
        ...
        type=str,
        default="detection",
        choices=["detection", "tracking", "smoothing"],
    )
    parser.add_argument(
        "--eval-type",
        type=str,
        default="detection",
        choices=["nuscenes", "giou", "fp", "all"],
    )
    parser.add_argument(
        "--config",
        type=str,
        default="/home/s0001668/workspace/thesis-autoann/AutoAnnSmoother/configs/training_config.yaml",
    )
    parser.add_argument("--giou", type=float, default="0.0")
    parser.add_argument(
        "--save-dir", type=str, default="/home/s0001668/workspace/storage/smoothing/"
    )
