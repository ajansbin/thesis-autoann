...
def main(data_path:str, version:str, split:str, result_path:str, type:str, config:str, save_path:str):
    conf = load_config(config)

    tracking_results = ZodTrackingResults(result_path, conf, version, split, data_path)

    evaluate_zod_nuscenes_style(tracking_results, type, save_path)


def evaluate_zod_nuscenes_style(tracking_results, type, save_path):
    ...
    for seq_id in gt_frames:
        ...
        for box in gts[seq_id]:
            if box['detection_name'] != 'Vehicle':
                print(box['detection_name'])
...
if __name__ == '__main__':
    ...
    main(args.data_path, args.version, args.split, args.result_path, args.type, args.config, args.save_dir)