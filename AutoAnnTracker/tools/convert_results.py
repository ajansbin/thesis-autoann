...
def main(result_path, root_path, version, split, save_path, type, convert_to):
    ...
    for scene_name in tqdm.tqdm(zod_seq.get_split(split), leave=True):
        ...
        #for lidar_frame, key in zip(lidar_frames[:-1], data['results'].keys()):
        #for lidar_frame, key in zip(lidar_frames[:-1], data['results'].keys()):
        ...
        #print('key', key)
            ...
            for i, b in enumerate(data['results'][key]):
                ...
                #result_world_coord['results'][key][i]['translation'] = list(box.center)
                #result_world_coord['results'][key][i]['rotation'] = box.orientation.elements
