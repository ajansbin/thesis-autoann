...
def main(data_path:str, track_results:str, save_dir:str, tag:str, seq_ids:str, nr_frames:int, no_tracks:bool):
    
    seq_ids = [str(item) for item in seq_ids.split(',')]
    for seq_id in seq_ids:
        print('SEQ', seq_id)
        name = str(seq_id) if tag=='' else str(seq_id) + '_' + tag
        track_seq = ZodTrackSequence(data_path, track_results, save_dir, name, seq_id, no_tracks)
...
if __name__ == '__main__':
    ...
    main(args.data_path, args.track_results, args.save_dir, args.tag, args.seq_id, args.nr_frames, args.no_tracks)