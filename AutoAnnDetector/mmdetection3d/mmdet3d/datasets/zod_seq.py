...
class ZodSequenceDataset(Custom3DDataset):
    ...
    def __init__(
        ...
        if self.anonymization_mode != BLUR:
            self._rename_image_paths(lambda x: x.replace(BLUR, self.anonymization_mode))
        if self.use_png:
    ...
    def get_data_info(self, index):
        ...
        input_dict = dict(
            ...
            sample_idx=os.path.basename(info["lidar_path"]),
        ...
        if self.modality["use_camera"]:
            ...
            if self.single_cam_mode:
                ...
                cam_input["img_info"] = dict(filename=cam_input["img_filename"])
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        for sample_id, det in enumerate(mmcv.track_iter_progress(results)):
            annos = []
            sample_token = os.path.basename(self.data_infos[sample_id]["lidar_path"])
    ...
    def _obj_to_zod(
        self, frame_id: str, box3d: np.ndarray, label: int, score: float = -1.0
