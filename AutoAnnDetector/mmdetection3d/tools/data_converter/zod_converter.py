...
def create_zod_infos(root_path,
    ...
    """
    if sequences:
        zod = ZodSequences(root_path, version)
        train_scenes = zod.get_split('train')
        val_scenes = zod.get_split('val')

        train_scenes = set(get_available_scenes(zod, train_scenes))
        val_scenes = set(get_available_scenes(zod, val_scenes))
        
        train_infos = _fill_seq_infos(zod, train_scenes, max_sweeps=max_sweeps)
        val_infos = _fill_seq_infos(zod, val_scenes, max_sweeps=max_sweeps)
...
def _fill_seq_infos(zod: ZodSequences, sequences: List[str], max_sweeps=10):
    
    infos = []

    for scene_name in mmcv.track_iter_progress(sequences):
        seq = zod[scene_name]
        lidar_frames = seq.info.get_lidar_frames(lidar=Lidar.VELODYNE)
        lidar_key_frame = seq.info.get_key_lidar_frame(lidar=Lidar.VELODYNE)
        
        #temporary solution to assert (selfmin <= np.min(target_ts)) and (
        for lidar_frame in lidar_frames[:-1]:
            mmcv.check_file_exist(lidar_frame.filepath)
            lidar_calib = seq.calibration.lidars[Lidar.VELODYNE]
            ...
            info = {
                ...
                'frame_id': scene_name, #what should this be
                'sweeps': [],
                'cams': dict(),
                ...
                'timestamp_datetime': lidar_frame.time,
            }
            ...
            for cam in cameras:
                cam_calib = seq.calibration.cameras[cam]
                cam_info = obtain_sensor2lidar(
                    seq.info.get_camera_frame(lidar_frame.time, Anonymization.BLUR, cam),
                    cam_calib,
                    core_ego_pose,
                    core_lidar2ego,
                    seq.ego_motion,
            ...
            #ADD FOR DIFF NUMBER OF SWEEPS
            info['sweeps'] = [
                #obtain_sensor2lidar(lf, lidar_calib, core_ego_pose, core_lidar2ego,
                #                    seq.ego_motion, 'lidar')
                #for lf in frame.get_lidar_frames(num_before=max_sweeps)[:-1]
                obtain_sensor2lidar(lidar_frame, lidar_calib, core_ego_pose, core_lidar2ego,
                                    seq.ego_motion, 'lidar')
                #for lf in seq.info.get_lidar_frame(lidar_frame.time, lidar=Lidar.VELODYNE)[:-1]
            ]

            #temporarly solving no annotations avaliable
            use_annotations = False 
            if lidar_key_frame != lidar_frame or not use_annotations:
                annos = []
                
            else:
                annos = seq.get_annotation(AnnotationProject.OBJECT_DETECTION)
            locs = np.array([
...
def export_2d_annotation(root_path, 
                         ...
                         mono3d=True, 
                         sequences=False):
    """Exsequencesport 2d annotation from the info file and raw data.
    ...
    if sequences:
        zod = ZodSequences(root_path, version=version)
    else:
    ...
    for info in mmcv.track_iter_progress(infos):
        #sample_id = info['seq_id'] if sequences else info['frame_id']
        sample_id = info['frame_id']
        sample = zod[sample_id]
        #if sequences:
        #    sample_id = info['seq_id']
        #    seq = zod[sample_id]
        #else:
        #     sample_id = info['frame_id']
        #    frame = zod[sample_id]
        ...
        for cam in camera_types:
            image_id = f"camera_{cam.value}_{sample_id}"
            ...
            if sequences:
                time = info['timestamp_datetime']
                width = sample.info.get_camera_frame(time, Anonymization.BLUR).width
                height = sample.info.get_camera_frame(time, Anonymization.BLUR).height
            else:
                width = sample.get_camera_frame().width
                height = sample.get_camera_frame().height
...
def get_2d_boxes(image_id, info, cam_info, mono3d=True):
    ...
    Return:
        list[dict]: List of 2D annotation record that belongs to the input
            `sample_data_token`.
    """    
    #if info['gt_boxes_2d'].size == 0: #if no annotation avaliable for frame
    #    return []
    records = []
