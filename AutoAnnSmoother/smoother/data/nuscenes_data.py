...
class NuscTrackingResults(TrackingResults):
    def __init__(
        ...
        super(NuscTrackingResults, self).__init__(
            tracking_results_path, config, version, split, data_path
        )
        ...
        self.nusc = (
            NuScenes(version=version, dataroot=data_path, verbose=True)
            if not nusc
            else nusc
        )
        ...
        self.scene2split = {
            self.nusc.scene[i]["token"]: self._get_scene2split(
                self.nusc.scene[i]["name"]
            )
            for i in range(len(self.nusc.scene))
        }
        self.split_scene_token = [
            scene_token
            for scene_token in self.scene2split
            if self.scene2split[scene_token] == self.split
        ]
