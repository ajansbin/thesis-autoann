...
def process_sequence(sequence_token, tracking_results, N_max, save_dir):
    ...
    for track_key, point_cloud_list in track_point_clouds.items():
        ...
        save_path = os.path.join(save_dir, f"point_clouds_{sequence_token}_{tracking_id}.npy")
...
def preprocess_and_save_point_clouds(tracking_results, save_dir):
    ...
    with multiprocessing.Pool(processes=10) as pool:
        results = [
            pool.apply_async(process_sequence, (sequence_token, tracking_results, N_max, save_dir))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--version', type=str, default="full", help="one of [full, mini]")
    parser.add_argument('--split', type=str, default="val")
    parser.add_argument('--out-dir', type=str, default="/storage/preprocessed")
    parser.add_argument('--tracking-result-path', type=str, default="/home/s0001671/workspace/storage/results/nuscenes-centerpoint-valsplit-trackresults.json")
    parser.add_argument('--verbose', type=bool, default=True)


    args = parser.parse_args()

    preprocess(args.data_path, args.version, args.split, args.out_dir, args.tracking_result_path)